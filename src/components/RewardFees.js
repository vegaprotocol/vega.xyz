  ...
  useEffect(() => {
    async function fetchRewardFees() {
      let response = await fetch(
        "https://api.n12.testnet.vega.xyz/api/v2/rewards?pagination.first=2000"
      );
      ...
      response.rewards.edges.forEach((node) => {
        ...
        if (epoch <= max_epoch && epoch > max_epoch - period) {
          if (node.node.rewardType === "ACCOUNT_TYPE_REWARD_MAKER_PAID_FEES") {
      ...
      setrewardValues({
        makerPaidFeeReward: (makerPaidFeeReward /= period) + ` $VEGA`,
        lpFeeReward: (lpFeeReward /= period) + ` $VEGA`,
        marketProposerReward: (marketProposerReward /= period) + ` $VEGA`,
      });
    }
    fetchRewardFees();
  }, []);
