import React from "react";
import Moshed from "../video/moshed.mp4";
import styled from "styled-components";

const StarPath = styled.path`
  fill: #000;

  .dark & {
    fill: #fff;
  }
`;

const BackgroundPath = styled.path`
  fill: #fff;

  .dark & {
    fill: #000;
  }
`;

const StarCrossed = () => {
  return (
    <div className="w-[235px] h-[151px] relative mx-auto">
      <video
        autoPlay
        muted
        loop
        playsInline
        className="absolute left-0 top-0 w-full h-full object-cover"
      >
        <source src={Moshed} type="video/mp4" />
      </video>

      <svg
        xmlns="http://www.w3.org/2000/svg"
        x="0"
        y="0"
        viewBox="0 0 235 151"
        className="absolute left-0 right-0 top-0 h-auto"
      >
        <BackgroundPath d="M42.1 75.6C42.1 33.8 76 0 117.7 0H0v151.2h117.7c-41.7 0-75.6-33.9-75.6-75.6zM117.7 0c41.7 0 75.6 33.8 75.6 75.6 0 41.7-33.8 75.6-75.6 75.6H235V0H117.7z" />
        <path d="M123.3 107.5h-11.9v11.9h11.9v-11.9zM135.2 95.6h-11.9v11.9h11.9V95.6zM159 83.7h-11.9v11.9H159V83.7zM147.1 36.1h-11.9v47.6h11.9V36.1zM111.4 95.6H99.5v11.9h11.9V95.6zM99.5 36.1H87.6v59.6h11.9V36.1z" />
        <StarPath d="M108.6 43h-2.9v2.9h2.9V43z" />
        <StarPath d="M158.5 55.5h2.9v2.9h-2.9zM161.4 52.6h2.9v2.9h-2.9zM164.3 49.6h2.9v2.9h-2.9zM167.3 46.7h2.9v2.9h-2.9zM178.9 38H176v2.9h-2.9v2.9h-2.9v2.9h2.9v-2.9h2.9v-2.9h2.9V38h3v-3h-3zM184.8 32.1h-2.9V35h2.9v-2.9h2.9v-2.9h-2.9zM202.3 14.6h-2.9v2.9h-2.9v2.9h-3v3h-2.9v2.9h-2.9v2.9h2.9v-2.9h2.9v-2.9h3v-3h2.9v-2.9h2.9v-2.9h2.9v-2.9h-2.9z" />
        <StarPath d="M208.1 8.8h-2.9v2.9h2.9V8.8h3v-3h-3zM214 2.9h-2.9v2.9h2.9V2.9h2.9V0H214z" />
        <StarPath d="M27.1 118.9h-2.9v2.9h2.9v-2.9zM30 116v-3h-2.9v5.8H30V116zM24.2 121.8h-5.8v2.9h5.8v-2.9zM27.1 124.7h-2.9v2.9h2.9v-2.9zM30 127.7h-2.9v5.8H30v-5.8zM32.9 124.7H30v2.9h2.9v-2.9zM38.8 121.8H33v2.9h5.8v-2.9zM32.9 118.9H30v2.9h2.9v-2.9zM41.5 110.1h-2.9v2.9h2.9v-2.9zM44.4 107.2h-2.9v2.9h2.9v-2.9zM47.3 104.3h-2.9v2.9h2.9v-2.9zM50.2 101.4h-2.9v2.9h2.9v-2.9zM53.2 98.4h-2.9v2.9h2.9v-2.9zM56.1 95.5h-2.9v2.9h2.9v-2.9zM59 92.6h-2.9v2.9H59v-2.9zM61.9 89.7H59v2.9h2.9v-2.9zM64.8 86.8h-2.9v2.9h2.9v-2.9zM67.8 83.8h-2.9v2.9h2.9v-2.9zM70.7 80.9h-2.9v2.9h2.9v-2.9zM73.6 78h-2.9v2.9h2.9V78zM76.5 75.1h-2.9V78h2.9v-2.9zM79.4 72.2h-2.9v2.9h2.9v-2.9zM82.4 69.2h-2.9v2.9h2.9v-2.9zM85.3 66.3h-2.9v2.9h2.9v-2.9zM88.2 63.4h-2.9v2.9h2.9v-2.9zM91.1 60.5h-2.9v2.9h2.9v-2.9zM94 57.6h-2.9v2.9H94v-2.9zM97 54.6h-3v2.9h3v-2.9zM99.9 51.7H97v2.9h2.9v-2.9zM102.8 48.8h-2.9v2.9h2.9v-2.9zM105.7 45.9h-2.9v2.9h2.9v-2.9z" />
        <StarPath d="M2.9 21.6H0v2.9h2.9v-2.9zM5.8 18.7H2.9v2.9h2.9v-2.9zM5.8 24.5H2.9v2.9h2.9v-2.9zM8.8 21.6h-3v2.9h2.9v-2.9z" />
        <StarPath d="M229.2 94h-2.9v2.9h2.9V94zM232.1 91.1h-2.9V94h2.9v-2.9zM232.1 96.9h-2.9v2.9h2.9v-2.9zM235.1 94h-2.9v2.9h2.9V94z" />
        <StarPath d="M196.2 132.5h-2.9v2.9h2.9v-2.9zM199.2 129.5h-2.9v2.9h2.9v-2.9zM199.2 135.4h-2.9v2.9h2.9v-2.9zM202.1 132.5h-2.9v2.9h2.9v-2.9z" />
      </svg>
    </div>
  );
};

export default StarCrossed;
